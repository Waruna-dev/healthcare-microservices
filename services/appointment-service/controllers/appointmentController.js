...
const checkAndCancelExpiredAppointments = async (appointments) => {
  ...
  for (let appt of appointments) {
    ...
    if (appt.startTime) {
      ...
      // Pending appointment that has passed its start time
      if (appt.status === "pending" && now > apptDate) {
        shouldCancel = true;
        cancelReason =
          "Auto-cancelled: Appointment time passed without doctor acceptance.";
      }

      // Accepted appointment but payment not made at least 10 minutes before start
      if (appt.status === "accepted" && appt.paymentStatus === "pending") {
        const tenMinutesBeforeStart = new Date(
          apptDate.getTime() - 10 * 60 * 1000,
        );
        if (now > tenMinutesBeforeStart) {
          shouldCancel = true;
          cancelReason =
            "Auto-cancelled: Patient did not complete payment before the 10-minute deadline.";
        }
      }
    }
...
const runAutoCancellationTask = async () => {
  try {
    ...
    // OR accepted appointments with pending payment that are within time
...
const processPaymentRequest = async (
  ...
  try {
    const response = await fetch(
      `${process.env.PAYMENT_SERVICE_URL || "http://localhost:5004"}/api/payments/create`,
      {
        method: "POST",
...
const createTelemedicineSession = async (
  ...
  try {
    const response = await fetch(
      `${process.env.TELEMEDICINE_SERVICE_URL || "http://localhost:5018"}/api/telemedicine/create`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      try {
        ...
        console.log(
          `📎 Uploaded ${uploadedReports.length} files to Cloudinary`,
        );
      } catch (uploadError) {
  ...
  try {
    ...
    if (status === "accepted") {
      ...
      if (appointment.startTime) {
        ...
        console.log(
          `📅 Set payment deadline for ${appointment._id}: ${deadline}`,
        );
      }
    }
  ...
  try {
    ...
    if (now > tenMinutesBeforeStart) {
      ...
      appointment.rejectionReason =
        "Auto-cancelled: Payment not completed before the 10-minute deadline.";
      await appointment.save();
      return res.status(400).json({
        success: false,
        message:
          "Payment window expired (10 minutes before start). Appointment cancelled.",
      });
    }
...
const completeAppointment = async (req, res) => {
  try {
    ...
    if (status === "no_show") {
      ...
      appointment.rejectionReason =
        consultationNotes ||
        "Patient did not attend the scheduled consultation";
    ...
    if (status === "doctor_no_show") {
      ...
      appointment.rejectionReason =
        consultationNotes || "Doctor did not attend the scheduled consultation";
      appointment.completionStatus = "completed";
      appointment.consultationNotes = consultationNotes;
      await appointment.save();
    ...
    if (status === "completed") {
      ...
      if (bothConfirmed) {
        ...
        appointment.consultationNotes =
          consultationNotes ||
          "Telemedicine consultation completed successfully by both parties.";
        appointment.prescription = prescription || null;
        console.log(
          " Both parties confirmed! Appointment marked as completed.",
        );
      } else if (updated) {
      ...
      if (bothConfirmed) {
        response.message =
          "Appointment completed successfully! Both parties have confirmed.";
      } else if (
        userType === "doctor" &&
        ...
        response.message =
          "Doctor confirmed. Waiting for patient confirmation to complete the appointment.";
      } else if (
        userType === "patient" &&
        appointment.patientConfirmed &&
        !appointment.doctorConfirmed
        ...
        response.message =
          "Patient confirmed. Waiting for doctor confirmation to complete the appointment.";
      } else {
...
const checkSlotAvailability = async (req, res) => {
  try {
    ...
    const existingAppointment = await Appointment.findOne({
      ...
      startTime,
      status: {
        $in: ["pending", "accepted", "completed", "no_show", "doctor_no_show"],
      },
    });
...
const updateAppointment = async (req, res) => {
  try {
    ...
    console.log("   Removed Reports:", removedReportIds);
    ...
    if (appointment.status !== "pending") {
      return res.status(400).json({
        success: false,
        message:
          "Can only update pending appointments. Current status: " +
          appointment.status,
      });
    }
    ...
    if (req.files && req.files.length > 0) {
      try {
        ...
        const newReports = cloudinaryResults.map((result) => ({
          fileName: result.fileName,
          filePath: result.url,
          publicId: result.publicId,
          uploadDate: new Date(),
        ...
        console.log(`📎 Added ${newReports.length} new reports to Cloudinary`);
      } catch (uploadError) {
        console.error("Cloudinary upload error:", uploadError);
      }
    }
...
const getReportUrl = async (req, res) => {
  try {
    ...
    const report = appointment.uploadedReports.id(reportId);
    if (!report) {
      return res.status(404).json({
        success: false,
